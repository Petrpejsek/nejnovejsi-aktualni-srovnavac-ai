...
export default function CreateLandingPage() {
  ...
  const handleFormatJson = () => {
    ...
    try {
      ...
      console.error('❌ Error details:', {
        ...
        position: error instanceof SyntaxError ? (error as any).position : null
      })
      
      // Show first 200 chars with invisible character detection
      const firstChars = jsonPayload.substring(0, 200)
      const charsWithUnicode = firstChars.split('').map((c, i) => {
        const code = c.charCodeAt(0)
        if (code > 127 || (code < 32 && code !== 9 && code !== 10 && code !== 13)) {
          return `[${i}:\\u${code.toString(16).toUpperCase().padStart(4, '0')}]`
        }
        return c
      }).join('')
      
      console.log('🔍 First 200 chars with Unicode codes:', charsWithUnicode)
      console.log('🔍 AI watermark check - suspicious chars found:', !!jsonPayload.match(/[\uFEFF\u200B-\u200D\u2060-\u206F\uFE00-\uFE0F]/))
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Vytvořit Landing Page</h1>
        <p className="text-gray-600">
          Ruční vytvoření landing page pomocí JSON payloadu z AI farmy. Slouží k testování před automatizací.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
            <h3 className="text-sm font-medium text-gray-900 mb-2">📝 Poslední aktivita</h3>
            <p className="text-sm text-gray-600">
              Zde se v budoucnu zobrazí historie vytvořených landing pages...
            </p>
          </div>
        </div>
      </div>
