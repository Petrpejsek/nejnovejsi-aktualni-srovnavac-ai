...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Query 3: Ad costs (only when available)
    let costs: any = null
    try {
    ...
    const mapKey = (row: Ga4Row) =>
      [
        row.dateHourMinute,
        row.sessionDefaultChannelGroup || '(other)',
        row.source || '(not set)',
        row.medium || '(not set)',
        row.campaign || '(not set)',
        // strip query to path
        (row.landingPagePlusQueryString || '/').split('?')[0],
    ...
    return NextResponse.json({ ok: true, rows: acc.size })
  } catch (e: any) {
    console.error('ga4 ingest error', e)
