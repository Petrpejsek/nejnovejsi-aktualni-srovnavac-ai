...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const where: any = { companyId, type: 'invoice' }
    ...
    const rows = await prisma.billingRecord.findMany({
      ...
      select: { id: true, createdAt: true, amount: true, status: true, invoiceNumber: true }
    })

    if (forCsv) {
      const header = ['timestamp','invoiceNumber','amount','status'].join(',')
      const lines = rows.map(r => [r.createdAt.toISOString(), r.invoiceNumber || '', String(r.amount ?? ''), r.status || ''].join(','))
      return new NextResponse([header, ...lines].join('\n'), { headers: { 'Content-Type': 'text/csv' } })
    }

    return NextResponse.json({ items: rows })
  } catch (e) {
    console.error('internal invoices list error', e)
