...
export async function POST(req: NextRequest) {
  ...
  if (!body || typeof body.to !== 'string' || typeof body.template !== 'string') return NextResponse.json({ error: 'Invalid payload' }, { status: 400 })

  const url = `${PYTHON_API_URL}/admin/email/send-template-test`
